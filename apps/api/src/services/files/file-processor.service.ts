...
import { files, knowledgeNodes, knowledgeEdges } from '@context-engine/db/schema';
import { eq } from 'drizzle-orm';
import { v4 as uuidv4 } from 'uuid';
import * as fs from 'fs/promises';
import * as path from 'path';
import type { FileMetadata, ExtractedContent, NodeType } from '@context-engine/shared';
...
export const fileProcessorService = {
  ...
  async integrateIntoGraph(
    ...
    if (nodesToCreate.length > 0) {
      await db.insert(knowledgeNodes).values(nodesToCreate);
    }
